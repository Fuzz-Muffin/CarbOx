...
class group:
    def __init__(self, group, origin, bonded_pos, box, aux_vec= np.zeros(3), surface=0):
        ...
        if group == 'hydroxyl':
            ...
            # O O C H coords/Users/filip/Documents/Science/Software/new_composites/oxidise_cf.py
...
def find_surface_atoms(infile, box, pbc):
    ...
    pipeline.modifiers.append(ConstructSurfaceModifier(radius = 4.,
                                                       only_selected = False,
...
if __name__=="__main__":
    ...
    is_surface = find_surface_atoms(infile, box, pbc)
    ...
    if not silent:
        for t in unique_atypelabels:
            print(f'{(atypelabels==t).sum()} {t} atoms')
    ...
    if len(added_groups) > 0:
        tmp_pos = np.row_stack((apos, np.row_stack([g.positions for g in added_groups])))
